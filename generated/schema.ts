...
export class RecoverToken extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save RecoverToken entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type RecoverToken must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("RecoverToken", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): RecoverToken | null {
    return changetype<RecoverToken | null>(
      store.get_in_block("RecoverToken", id.toHexString())
    );
  }

  static load(id: Bytes): RecoverToken | null {
    return changetype<RecoverToken | null>(
      store.get("RecoverToken", id.toHexString())
    );
  }
